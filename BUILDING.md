...
git apply /path/to/patches/etk-rpcs3-gtk-edition-0.8.4-dev.patch
```
...
## 3.5 Verify no feature was dropped (`scripts/verify-markers.sh`)

Cumulative patches are regenerated by hand (`git diff <base>..HEAD`), and twice a feature
has vanished in that step unnoticed: the ffs series published without `RSXTexture.cpp`
(the #11912 fix, corrected 2026-07-09), and the 0.7.x consolidation dropped
`GTK_PROBE_11912` — the TIU probe that localized that same bug — which stayed missing
through 0.8.3 until an unrelated disk audit found it. Both were invisible because the
build still compiled and ran; the feature was simply gone.

```sh
scripts/verify-markers.sh                                   # newest patch
scripts/verify-markers.sh patches/<file>.patch build/bin/rpcs3   # patch + binary
```

Greps every shipped feature's marker string out of the patch, and optionally out of the
built ELF (AppImages are extracted first — their payload is compressed, so `strings` on
the wrapper finds nothing). Exit 1 if anything is missing. **Run it before publishing a
patch or cutting a release**; add a line to the `MARKERS` list whenever a new gated
feature lands.
...
use `git checkout a1deb2921` — v0.0.41-19638 — with `patches/etk-rpcs3-gtk-edition-0.8.4-dev.patch`.)
